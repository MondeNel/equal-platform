...
export default function CandleChart({ symbol, livePrice, entry, takeProfit, stopLoss, onEntry, onTP, onSL, openTrades = [], pendingOrders = [], onPriceUpdate }) {
  ...
  useEffect(() => {
  console.log('CandleChart livePrice:', livePrice);
}, [livePrice]);
  ...
  useEffect(() => {
    const id = setInterval(() => {
      ...
      const noise = (Math.random() - 0.5) * realPrice * 0.0005;  // increased noise
      const pull  = (realPrice - prev) * 0.5;                    // stronger pull
      ...
      setCandles(prev => {
        ...
        // Complete candle every 10 seconds (25 ticks * 400ms = 10s)
