...
async def check_and_execute_limit_orders():
    """Background worker that checks pending limit orders and executes them when target buy price is reached"""
    while True:
        try:
            async with AsyncSessionLocal() as db:
                ...
                if orders:
                    ...
                    for order in orders:
                        # Get current price for the buy exchange
                        current_buy_price = prices.get(order.buy_exchange, {}).get(order.symbol, 0)
                        target_price = float(order.target_spread_pct)  # This stores the target buy price
                        
                        # Check if current price <= target price (for BUY orders)
                        if current_buy_price > 0 and current_buy_price <= target_price:
                            # Execute the trade at current market prices
                                    ...
                                    try:
                                        ...
                                        if success:
                                            ...
                                            await manager.send_personal_message({
                                                ...
                                                "profit": float(pnl),
                                                "executed_price": current_buy_price,
                                                "target_price": target_price,
                                                "message": f"Limit order executed at price {current_buy_price}! Profit: ZAR {pnl:.2f}"
                                            }, str(order.user_id))
                ...
                for order in expired_orders:
                    ...
                    await manager.send_personal_message({
                        ...
                        "symbol": order.symbol,
                        "amount": float(order.amount),
                        "target_price": float(order.target_spread_pct),
                        "message": f"Limit order expired. Target price of {order.target_spread_pct} was not reached."
                    }, str(order.user_id))
