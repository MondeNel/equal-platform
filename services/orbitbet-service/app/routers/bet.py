...
async def place_bet(
    ...
    try:
        ...
        # Reserve stake in wallet
        reservation_ref = str(uuid.uuid4())
        await reserve_stake(user_id_str, request.stake, reservation_ref)
        
        # Determine overall outcome and generate round results
        overall_win = determine_outcome()
        round_results = generate_round_results(overall_win)
        
        # Get initial dummy price for round 1
        current_price = await get_current_price(request.symbol)
        logger.info(f"Initial price for {request.symbol}: {current_price}")
        
        # Get or create stats
        stats = await get_or_create_stats(db, user_uuid)
        
        # Create bet with predetermined outcome
        bet = Bet(
            user_id=user_uuid,
            ...
            status="ACTIVE",
            predetermined_outcome=overall_win,
            round_1_pred=round_results[0],
            round_2_pred=round_results[1],
            round_3_pred=round_results[2],
...
async def resolve_round_endpoint(
    ...
    try:
        ...
        # Get predetermined round result
        if current_round == 1:
            round_result = bet.round_1_pred
            entry_price = bet.round_1_price
        elif current_round == 2:
            round_result = bet.round_2_pred
            entry_price = bet.round_2_price
        else:
            round_result = bet.round_3_pred
            entry_price = bet.round_3_price
        
        if entry_price is None:
            raise HTTPException(status_code=400, detail="Round entry price not found")
        
        # Generate exit price that matches the result given the user's chosen direction
        exit_price = generate_price_for_round(
            float(entry_price),
            request.chosen_direction,
            round_result
        )
        
        # Store exit price and set entry price for next round
        if current_round == 1:
            bet.round_1 = round_result
            bet.round_1_exit_price = Decimal(str(exit_price))
            bet.round_2_price = Decimal(str(exit_price))
        elif current_round == 2:
            bet.round_2 = round_result
            bet.round_2_exit_price = Decimal(str(exit_price))
            bet.round_3_price = Decimal(str(exit_price))
        else:
            bet.round_3 = round_result
            bet.round_3_exit_price = Decimal(str(exit_price))
        ...
        if is_complete:
            ...
            if bet.predetermined_outcome:
                ...
                # Apply winnings: release stake + profit
                pnl = payout - bet.stake
                success = await release_stake(user_id_str, bet.stake, bet.reservation_id, pnl)
                if not success:
                ...
                # Release stake as loss (pnl = -stake)
                pnl = -bet.stake
                success = await release_stake(user_id_str, bet.stake, bet.reservation_id, pnl)
                if not success:
                    raise HTTPException(status_code=502, detail="Wallet release failed")
        ...
        price_movement = ((exit_price - float(entry_price)) / float(entry_price)) * 100
